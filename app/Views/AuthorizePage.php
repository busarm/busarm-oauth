...
class AuthorizePage extends View
{
    public function __construct(protected AuthorizePageDto|BaseDto|array|null $data = null, protected $headers = array())
    {
    }
                    ...
                    <div class="form">
                        ...
                        <form id="login-form" class="form" method="post" action="<?= $action ?? null ?>">
                            ...
                            <?php if (!empty($this->data->user_name) || !empty($this->data->user_email)) : ?>
                                <div>
                                    <div>
                                        <?php if ($this->data->user_name) : ?>
                                            <strong><?= $this->data->user_name ?> </strong>
                                            <div><?= $this->data->user_email ?> </div>
                                        <?php else : ?>
                                            <strong><?= $this->data->user_email ?> </strong>
                                        <?php endif ?>
                                    </div>
                                </div>
                                <?php if (!empty($this->data->scopes)) : ?>
                                    <h4>Grant permission to do the following: </h3>
                                        <ul>
                                            <?php foreach ($this->data->scopes as $scope) : ?>
                                ...
                                <div class="auth-buttons">
                                    ...
                                    <div style="padding: 8px; color:darkcyan; text-decoration: none;">
                                        <a style="color:darkcyan; text-decoration: none;" href="/authorize/logout?redirect_url=<?= urlencode($this->data->action) ?? null ?>">Use another account</a>
                                    </div>
                                </div>
                        </form>
                    </div>
